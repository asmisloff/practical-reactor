...
public class c6_CombiningPublishers extends CombiningPublishersBase {
    ...
    public void task_executor() {
        Flux<Void> tasks = taskExecutor().flatMap(it -> it);
    ...
    public void streaming_service() {
        Flux<Message> messageFlux = streamingService().flatMapMany(it -> it);
    ...
    public void task_executor_again() {
        Flux<Void> tasks = taskExecutor().concatMap(it -> it);
    ...
    public void mail_box_switcher() {
        Flux<Message> myMail = mailBoxPrimary()
            .switchOnFirst((signal, prim) -> {
                Message msg = signal.get();
                return (msg.payload.contains("spam") || msg.metaData.contains("spam"))
                    ? mailBoxSecondary()
                    : prim;
            });
    ...
    public void acid_durability() {
        Flux<String> committedTasksIds = tasksToExecute()
            .flatMapSequential(task -> task.doOnNext(this::commitTask));
    ...
    public void car_factory() {
        Flux<Car> producedCars = Flux
            .zip(carChassisProducer(), carEngineProducer())
            .flatMap(t -> {
                Car car = new Car(t.getT1(), t.getT2());
    ...
    public Mono<String> chooseSource() {
        return Mono.defer(() -> {
            var s = sourceRef.get();
            if ("A".equals(s)) return sourceA();
            if ("B".equals(s)) return sourceB();
            return Mono.empty();
        });
    }
    ...
    public void cleanup() {
        ...
        Flux<String> stream = Flux.usingWhen(StreamingConnection.startStreaming(),
                                             Function.identity(),
                                             res -> StreamingConnection.closeConnection());
